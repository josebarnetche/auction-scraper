#!/usr/bin/env python3
"""Patch index.html to add location-based features UI (handles escaped slashes)."""
...
# The linter is escaping forward slashes in CSS classes
# Look for the pattern with escaped slashes

# Check if already patched
if 'near-me-btn' in content:
    print('Already patched!')
    exit(0)

# Find the filter section using regex to handle the escaped slashes
pattern = r'<div class="grid grid-cols-1 md:grid-cols-3 gap-4">\s*<!-- Source Filter -->'

if re.search(pattern, content):
    # Find and replace the filter section
    old_pattern = r'''<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <!-- Source Filter -->
                    <div>
    ...
    # Read content line by line and find the section
    lines = content.split('\n')
    start_idx = None
    end_idx = None

    for i, line in enumerate(lines):
        if 'grid grid-cols-1 md:grid-cols-3 gap-4' in line and 'Source Filter' in lines[i+1]:
            start_idx = i
        if start_idx and '<!-- TOP OPPORTUNITIES SECTION -->' in line:
            end_idx = i
            break

    if start_idx and end_idx:
        # Build the new section
        new_section = '''                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <!-- Source Filter -->
                    <div>
                        <label class="text-xs uppercase tracking-wider text-white/40 mb-2 block">Source</label>
                        <select id="source-filter" class="w-full bg-[#1a1a1a] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-yellow-500/50" onchange="applyFilters()">
        ...
        print('Filter section patched successfully!')
    else:
        print(f'Could not find section boundaries: start={start_idx}, end={end_idx}')
else:
    print('Could not find filter section pattern')
