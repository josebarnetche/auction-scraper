"""DeMotores scraper for vehicle market prices."""
...
@dataclass
class DeMotoresVehicle:
    """DeMotores vehicle price data."""
    brand: str
    model: str
    year: int
    price_usd: float
...
class DeMotoresScraper:
    """Scraper for DeMotores.com.ar vehicle prices."""

    BASE_URL = "https://www.demotores.com.ar"
    ...
    async def search_vehicle(
        ...
        if model:
            ...
            url = f"{self.BASE_URL}/autos/{brand_slug}/{model_slug}/usados"
        else:
            url = f"{self.BASE_URL}/autos/{brand_slug}/usados"
        ...
        try:
            ...
            if response.status_code != 200:
                logger.debug(f"DeMotores returned {response.status_code} for {url}")
                # Try search fallback
                return await self._search_query(brand, model, year, limit)

            soup = BeautifulSoup(response.text, "lxml")
            return self._parse_results(soup, brand, model, year, limit)

        except Exception as e:
            logger.error(f"DeMotores search error: {e}")
            return []
    ...
    async def _search_query(
        ...
        """Fallback search using query string."""
        query = f"{brand} {model}".strip()
        if year:
            query = f"{query} {year}"

        encoded_query = quote_plus(query)
        url = f"{self.BASE_URL}/autos/usados?q={encoded_query}"
        ...
        try:
            ...
            logger.debug(f"DeMotores query search error: {e}")
            return []
    ...
    def _parse_results(
        ...
        # Find listing cards
        cards = soup.select(
            ".car-card, .vehicle-card, [class*='listing'], "
            "[class*='result'], article, .item"
        )
        ...
        for card in cards[:limit * 2]:
            try:
                ...
                logger.debug(f"Error parsing DeMotores card: {e}")
